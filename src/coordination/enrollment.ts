...
export function launcherEnrollment(
  ...
  const authorized = launcherCredential(launcherSecret);
  return (credential: string, input: unknown) => {
    if (!authorized(credential))
      throw new CoordinationError("forbidden", "Launcher enrollment requires its separate credential");
    return enroll(store, input);
  };
}

export function launcherCredential(launcherSecret: string) {
  ...
  return (credential: string) => timingSafeEqual(digest, Buffer.from(secretHash(credential), "hex"));
}

function enroll(store: CoordinationStore, input: unknown) {
