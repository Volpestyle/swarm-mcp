...
test("existing peer receives one durable assignment and completes from another Node process", async () => {
  ...
  try {
    ...
    const child = Bun.spawn(
      [Bun.which("node")!, fixture, path, JSON.stringify(worker)],
    ...
    expect(store.task("scope", result.taskId)?.status).toBe("completed");
