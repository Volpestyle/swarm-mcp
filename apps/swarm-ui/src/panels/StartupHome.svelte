<script lang="ts">
  ...
  let appIdentity: AppIdentity | null = null;
  let fallbackAppIdentity: AppIdentity | null = null;
  ...
  $: appIdentity = buildProvenance ? appIdentityFromProvenance(buildProvenance) : fallbackAppIdentity;
  $: provenanceModeLabel = appIdentity ? runKindLabel(appIdentity.runKind) : 'PREVIEW';
  $: provenanceVariantLabel = appIdentity ? appIdentity.appVariant.toUpperCase() : 'UNKNOWN';
  $: provenanceVersionLabel = appIdentity ? `v${appIdentity.appVersion}` : 'version unknown';
  $: provenanceRunPathLabel = appIdentity ? compactRunPath(appIdentity) : 'runpath unavailable';
  $: provenanceDockLabel = appIdentity
    ? appIdentity.dockStatus === 'this-bundle-run'
      ? 'this bundle run'
      : appIdentity.dockStatus === 'not-this-run'
        ? 'not this run'
        : 'unknown'
    : 'unknown';
  $: provenanceTitle = buildProvenance
    ...
    : appIdentity
      ? fallbackProvenanceTooltip(appIdentity, buildProvenanceError)
  ...
  function provenanceTooltip(provenance: BuildProvenance): string {
    const identity = appIdentityFromProvenance(provenance);
    return [
      `App: swarm-ui ${identity.appVariant.toUpperCase()}`,
      ...
      `Dock status: ${identity.dockStatus}`,
      `Working directory: ${provenance.currentWorkingDirectory}`,
      `Source root: ${provenance.sourceRoot}`,
    ].join('\n');
  }

  function fallbackProvenanceTooltip(identity: AppIdentity, error: string | null): string {
    return [
      `App: swarm-ui ${identity.appVariant.toUpperCase()}`,
      `Mode: ${runKindLabel(identity.runKind)} (${identity.runKind}, ${identity.buildProfile})`,
      `Version: ${identity.appVersion}`,
      `Run path: ${identity.executablePath}`,
      `Source root: ${identity.sourceRoot || 'unknown'}`,
      'Tauri provenance unavailable in browser preview.',
  ...
  async function loadBuildProvenance(): Promise<void> {
    try {
      ...
      fallbackAppIdentity = null;
      buildProvenanceError = null;
    } catch (err) {
      buildProvenance = null;
      buildProvenanceError = err instanceof Error ? err.message : String(err);
      fallbackAppIdentity = browserPreviewIdentity(packageInfo.version, '/Users/mathewfrazier/Desktop/swarm-mcp-lab');
      console.warn('[StartupHome] failed to load build provenance:', err);
    }
  }
  ...
  <div class="home-shell">
    ...
    <section class="home-content">
      ...
      <div class="summary-strip" aria-label="Home summary">
        <span class="summary-item summary-item--build" data-testid="home-app-identity-strip" title={provenanceTitle}>
          <em>{provenanceVariantLabel}</em> {provenanceVersionLabel} · {provenanceModeLabel} · run <em>{provenanceRunPathLabel}</em>
        </span>
        <span class="summary-item" title={provenanceTitle}>dock <em>{provenanceDockLabel}</em></span>
