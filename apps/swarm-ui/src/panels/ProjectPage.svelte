<script lang="ts">
  ...
  import {
    ...
    createDraftTaskRow as createModelDraftTaskRow,
    groupTaskBoardRows,
    parsePlanRows as parseModelPlanRows,
    ...
    roleToLaunchRole,
    setAllTaskSelection as setAllTaskSelectionInRows,
    statusAfterLaunch,
    syncTaskBoardRows as syncModelTaskBoardRows,
    taskBoardSourceKey as buildTaskBoardSourceKey,
    taskLaunchInstructions as buildTaskLaunchInstructions,
    taskLaunchLabel as buildTaskLaunchLabel,
    taskLaunchName,
    taskRowsForProofPack as modelTaskRowsForProofPack,
    toggleTaskSelection as toggleTaskSelectionInRows,
    updateTaskBoardRow as updateTaskBoardRows,
    type TaskBoardInstanceRuntimeInput,
    type TaskBoardLaunchHarness,
  ...
  $: projectTasks = projectTasksForProject(tasks, project, projectMemberIds);
  $: taskBoardLanes = buildTaskBoardLanes(projectTasks);
  $: nextTaskBoardSourceKey = buildTaskBoardSourceKey(project.id, projectTasks);
  ...
  function createDraftTaskRow(input: Partial<TaskBoardRow> = {}): TaskBoardRow {
    return createModelDraftTaskRow({
      ...input,
  ...
  function syncTaskBoardRows(key: string, taskList: Task[]): void {
    taskBoardRows = syncModelTaskBoardRows(project.id, taskList, taskBoardRows);
    taskBoardSourceKey = key;
  }

  function updateTaskBoardRow(id: string, patch: Partial<TaskBoardRow>): void {
    taskBoardRows = updateTaskBoardRows(taskBoardRows, id, patch);
  }
  ...
  function toggleTaskSelection(id: string, selected: boolean): void {
    taskBoardRows = toggleTaskSelectionInRows(taskBoardRows, id, selected);
  }

  function setAllTaskSelection(selected: boolean): void {
    taskBoardRows = setAllTaskSelectionInRows(taskBoardRows, selected);
  }
  ...
  function resetTaskLaunchState(rowId: string): void {
    ...
    updateTaskBoardRow(rowId, resetTaskLaunchStateForRow(row));
  ...
  function reassignTaskRow(rowId: string): void {
    ...
    updateTaskBoardRow(rowId, reassignTaskRowToBulk(row, bulkProvider, bulkRole));
  ...
  function importPlanRows(): void {
    ...
    taskBoardError = null;
    launchActionMessage = null;
    const rows = parseModelPlanRows(planImportText, project.id, bulkProvider, bulkRole);
    if (rows.length === 0) {
  ...
  function applySelectedAssignment(): void {
    ...
    taskBoardRows = applySelectedAssignmentToRows(taskBoardRows, bulkProvider, bulkRole);
  ...
  function taskLaunchLabel(row: TaskBoardRow): string {
    return buildTaskLaunchLabel(project.id, row, 'project_task_board', 'may_1_s4');
  }
  ...
  function taskLaunchInstructions(
    ...
    return buildTaskLaunchInstructions({
      project,
      row,
      harness,
      launchRole,
  ...
  function taskRowsForProofPack(): ProofPackTaskRow[] {
    return modelTaskRowsForProofPack(taskBoardRows, taskBoardInstanceStates);
  }
          ...
          <div class="post-session-policy-grid">
            <label>
              ...
              <select bind:value={backgroundHarness} disabled={!backgroundWorkOptIn}>
                ...
                <option value="hermes">Hermes (Nous)</option>
