<script lang="ts">
  ...
  import type { BrowserCatalog, BrowserContext, BrowserSnapshot, BrowserTab, ChromeTabCandidate, ProjectSpace } from '../lib/types';
  import { startupPreferences, startupThemeProfile } from '../stores/startup';
  import { activeScope, instances, setScopeSelection } from '../stores/swarm';
  import {
  ...
  const dispatch = createEventDispatcher<{
    ...
    importFile: void;
	    organize: void;
	    openSettings: void;
	    browserCatalog: { catalog: BrowserCatalog; focusContextId: string | null; message: string | null };
	  }>();
  ...
  const resourceLanes: ResourceLane[] = [
    ...
    { id: 'browser', label: 'Browser', detail: 'Launch a managed Chrome surface on the canvas.', enabled: true, iconLabel: 'Chrome browser', iconSrc: chromeLogo },
  ];
  ...
  $: if (browserOpenSignal !== lastBrowserOpenSignal) {
    ...
    browserError = null;
    projectError = null;
    kitMessage = 'Choose New Tab or Import Tabs for the active canvas.';
    void loadBrowserCatalog();
  }
  ...
  function handleDocumentPointerdown(event: PointerEvent): void {
    ...
    if (!target) return;
    if (target.closest('.top-strip') || target.closest('.project-modal')) return;
    kitOpen = false;
    infoOpen = false;
  }
  ...
  function browserScopeForCanvasAction(): string | null {
    const scope = browserScope.trim();
    if (!scope) {
  ...
  function applyBrowserCatalog(
    catalog: BrowserCatalog,
    focusContextId: string | null = null,
    message: string | null = null,
  ): void {
	    browserContexts = catalog.contexts ?? [];
	    browserTabs = catalog.tabs ?? [];
	    browserSnapshots = catalog.snapshots ?? [];
	    dispatch('browserCatalog', { catalog, focusContextId, message });
	  }
  ...
  async function refreshBrowserCatalog(): Promise<void> {
    const scope = browserScopeForCanvasAction();
    if (!scope) return;
    browserCatalogScope = scope;
    browserLoading = true;
    browserError = null;
    try {
      applyBrowserCatalog(await invoke<BrowserCatalog>('ui_refresh_browser_catalog', { scope }), null, 'Browser surfaces checked for stale contexts.');
    } catch (err) {
      browserError = err instanceof Error ? err.message : String(err);
    } finally {
      browserLoading = false;
  ...
  async function openManagedBrowserContext(): Promise<void> {
    await openManagedBrowserContextWithMessage('Managed browser context launched.');
  }

  async function openManagedBrowserContextFromCanvas(): Promise<void> {
    await openManagedBrowserContextWithMessage('Managed browser context launched on the canvas.');
  }

	  async function openManagedBrowserContextWithMessage(successMessage: string): Promise<void> {
	    const scope = browserScopeForCanvasAction();
	    if (!scope) return;
	    ...
	    try {
	      ...
	      const focusContext = catalog.contexts.find((context) => !previousIds.has(context.id))
	        ?? catalog.contexts[0]
	        ?? null;
	      applyBrowserCatalog(catalog, focusContext?.id ?? null, successMessage);
	      kitMessage = successMessage;
    } catch (err) {
      browserError = err instanceof Error ? err.message : String(err);
    } finally {
      browserLoading = false;
	  ...
	  async function loadChromeTabsForImport(): Promise<void> {
	    const scope = browserScopeForCanvasAction();
	    if (!scope) return;
	    browserCatalogScope = scope;
	    ...
	    try {
	      ...
	      kitMessage = 'Select the Chrome tabs to import into one canvas browser.';
    } catch (err) {
      browserError = err instanceof Error ? err.message : String(err);
    } finally {
  ...
  function toggleChromeTabSelection(tabId: string): void {
    ...
    selectedChromeTabIds = next;
	  ...
	  async function importSelectedChromeTabs(): Promise<void> {
	    const scope = browserScopeForCanvasAction();
	    if (!scope) return;
	    const selectedTabs = chromeTabCandidates.filter((tab) => selectedChromeTabIds.has(tab.id));
	    if (selectedTabs.length === 0) {
	      browserError = 'Select at least one Chrome tab to import.';
	      return;
	    }
	    const previousIds = new Set(browserContexts.map((context) => context.id));
	    browserCatalogScope = scope;
	    ...
	    try {
	      ...
	      const focusContext = catalog.contexts.find((context) => !previousIds.has(context.id))
	        ?? catalog.contexts[0]
	        ?? null;
	      applyBrowserCatalog(catalog, focusContext?.id ?? null, `${selectedTabs.length} Chrome tab${selectedTabs.length === 1 ? '' : 's'} imported into one browser surface.`);
	      kitMessage = `${selectedTabs.length} Chrome tab${selectedTabs.length === 1 ? '' : 's'} imported into one managed browser context.`;
    } catch (err) {
      browserError = err instanceof Error ? err.message : String(err);
    } finally {
      browserLoading = false;
  ...
  async function closeManagedBrowserContext(): Promise<void> {
    const scope = browserScopeForCanvasAction();
    if (!scope || !selectedBrowserContext) {
    ...
    try {
      const catalog = await invoke<BrowserCatalog>('ui_close_browser_context', {
        ...
        contextId: selectedBrowserContext.id,
	      ...
	      applyBrowserCatalog(catalog, null, 'Managed browser context closed.');
      ...
      browserError = err instanceof Error ? err.message : String(err);
		  ...
		  async function deleteManagedBrowserContext(): Promise<void> {
	    const scope = browserScopeForCanvasAction();
	    if (!scope || !selectedBrowserContext) {
	      browserError = 'Select a browser context first.';
	      return;
	    ...
	    try {
	      ...
	      applyBrowserCatalog(catalog, null, 'Managed browser context deleted from the canvas.');
	      kitMessage = 'Managed browser context deleted.';
	    } catch (err) {
	      browserError = err instanceof Error ? err.message : String(err);
		    } finally {
		      browserLoading = false;
    ...
    try {
      ...
      applyBrowserCatalog(catalog, selectedBrowserContext.id, 'Browser snapshot captured.');
...
<header class="top-strip" data-theme-id={themeId} aria-label="swarm-ui status strip">
        ...
        <div class="kit-dropdown" role="menu" aria-label="Workspace kit">
          ...
          <div class="kit-action-grid" aria-label="Workspace actions">
            ...
            <button type="button" role="menuitem" on:click={() => dispatchKitAction('launchChrome')}>
              ...
              <span>Fresh canvas surface</span>
            </button>
          ...
          <div class="kit-lane-list" aria-label="Resource lanes">
            {#each resourceLanes as lane (lane.id)}
              ...
              >
                <img src={lane.iconSrc ?? darkFolderAsset} alt="" aria-hidden="true" />
            ...
            <div class="browser-resource-panel" aria-label="Browser contexts">
              <header class="browser-panel-head">
                ...
                <div class="browser-panel-actions">
                  ...
                  <button type="button" disabled={chromeTabsLoading || browserLoading || !browserScope} on:click={loadChromeTabsForImport}>
                    {chromeTabsLoading ? 'Reading...' : 'Import Tabs'}
                  </button>
                  ...
                  <button
                    type="button"
                    disabled={browserLoading || !selectedBrowserContext}
                    on:click={deleteManagedBrowserContext}
                  >
                    Delete
                  </button>
                </div>
              ...
              {#if chromeTabCandidates.length > 0}
                <div class="chrome-import-panel" aria-label="Chrome tabs available to import">
                  <div class="chrome-import-head">
                    <strong>Open Chrome tabs</strong>
                    <button
                      type="button"
                      disabled={browserLoading || selectedChromeTabCount === 0}
                      on:click={importSelectedChromeTabs}
                    >
                      Import {selectedChromeTabCount}
                    </button>
                  </div>
                  <div class="chrome-import-list">
  ...
  <div class="strip-right">
    <div class="strip-info-shell">
      <button
        type="button"
        class="strip-info-button"
      ...
      {#if infoOpen}
        <div class="strip-info-popover" role="dialog" tabindex="-1" aria-label="Workspace information" on:pointerdown|stopPropagation>
          <header>
            <strong>Workspace</strong>
            <button type="button" aria-label="Close workspace information" on:click={closeInfo}>×</button>
          </header>
          <dl>
            <div><dt>Mode</dt><dd>{provenanceModeLabel}</dd></div>
            <div><dt>Version</dt><dd>{buildProvenance?.appVersion ?? 'unknown'}</dd></div>
            <div><dt>Channel</dt><dd title={channelLabel}>{channelLabel}</dd></div>
            <div><dt>Nodes</dt><dd>{instanceCount}</dd></div>
            <div><dt>Theme</dt><dd>{themeName}</dd></div>
            <div><dt>Branch</dt><dd>{buildProvenance?.gitBranch ?? 'unknown'}</dd></div>
            <div><dt>Commit</dt><dd>{buildProvenance?.gitCommit ?? 'unknown'}</dd></div>
            <div><dt>Source</dt><dd title={buildProvenance?.sourceRoot ?? ''}>{buildProvenance?.sourceRoot ?? 'unknown'}</dd></div>
            <div><dt>CWD</dt><dd title={buildProvenance?.currentWorkingDirectory ?? ''}>{buildProvenance?.currentWorkingDirectory ?? 'unknown'}</dd></div>
            <div><dt>Build</dt><dd title={provenanceTitle}>{provenanceInline}</dd></div>
          </dl>
        </div>
      {/if}
    </div>
...
<style>
  ...
  .chrome-import-panel {
    display: grid;
    gap: 8px;
    border: 1px solid color-mix(in srgb, var(--status-pending, #f9e2af) 34%, transparent);
    border-radius: 8px;
  ...
  .chrome-import-head button {
    border: 1px solid color-mix(in srgb, var(--status-pending, #f9e2af) 52%, transparent);
    border-radius: 6px;
    background: color-mix(in srgb, var(--status-pending, #f9e2af) 12%, transparent);
    color: var(--terminal-fg, #c0caf5);
    font: inherit;
    font-size: 10px;
    padding: 5px 8px;
  ...
  .chrome-import-head button:disabled {
    cursor: default;
    opacity: 0.48;
  }
  ...
  .chrome-import-list label.active {
    border-color: color-mix(in srgb, var(--status-pending, #f9e2af) 60%, transparent);
    background: color-mix(in srgb, var(--status-pending, #f9e2af) 10%, rgba(0, 0, 0, 0.26));
  }

  .chrome-import-list input {
    accent-color: var(--status-pending, #f9e2af);
  }

  .chrome-import-list span {
    min-width: 0;
    display: grid;
    gap: 2px;
  }

  .chrome-import-list strong,
  .chrome-import-list em {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
  ...
  .strip-info-button {
    ...
    border: 1px solid color-mix(in srgb, var(--node-border, #6c7086) 72%, transparent);
    border-radius: 999px;
    background: color-mix(in srgb, var(--terminal-bg, #1e1e1e) 70%, transparent);
    color: var(--terminal-fg, #c0caf5);
    font: inherit;
    font-size: 12px;
    font-weight: 800;
  ...
  .strip-info-button:hover,
  .strip-info-button[aria-expanded="true"] {
    border-color: color-mix(in srgb, var(--status-pending, #89b4fa) 62%, white 8%);
    box-shadow: 0 0 18px color-mix(in srgb, var(--status-pending, #89b4fa) 18%, transparent);
  }
  ...
  .strip-info-popover {
    ...
    display: grid;
    gap: 10px;
    border: 1px solid color-mix(in srgb, var(--node-border, #6c7086) 74%, transparent);
    border-radius: 8px;
    background: color-mix(in srgb, var(--node-header-bg, #2d2d2d) 88%, transparent);
    color: var(--terminal-fg, #c0caf5);
    padding: 12px;
  ...
  .strip-info-popover header strong {
    font-size: 11px;
    letter-spacing: 0.12em;
    text-transform: uppercase;
  }

  .strip-info-popover header button {
    width: 24px;
    height: 24px;
    border: 1px solid color-mix(in srgb, var(--node-border, #6c7086) 68%, transparent);
    border-radius: 6px;
    background: transparent;
    color: inherit;
    cursor: pointer;
  ...
  .strip-info-popover dl {
    display: grid;
    gap: 7px;
    margin: 0;
  }

  .strip-info-popover dl div {
    min-width: 0;
    display: grid;
  ...
  .strip-info-popover dt,
  .strip-info-popover dd {
    margin: 0;
    min-width: 0;
    overflow: hidden;
    text-overflow: ellipsis;
