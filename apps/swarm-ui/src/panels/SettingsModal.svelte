<script lang="ts">
  ...
  let daemonProcesses: DaemonProcessRow[] = [];
  ...
  $: scope = $startupPreferences.launchDefaults.scopePinned
    ? $startupPreferences.launchDefaults.scope
    : '';
  ...
  $: daemonProcesses = $systemLoadSnapshot?.daemon_processes ?? [];
  $: staleDaemonCount = daemonProcesses.filter((row) => row.stale).length;
  ...
  function formatDaemonRuntime(seconds: number): string {
    ...
    <div class="settings-body">
      ...
      <section>
        ...
        <div class="metrics-grid">
          ...
          <div><span>{daemonProcesses.length}</span><p class="metric-label">daemons</p></div>
        </div>

        <div class="settings-grid two-up">
          <div class="setting-card">
            <div class="setting-copy">
              <h4 class="card-label">Daemon Radar</h4>
              <p>{staleDaemonCount} stale detached daemon{staleDaemonCount === 1 ? '' : 's'} detected.</p>
            </div>
            <div class="setting-control action-stack">
              {#if !$systemLoadSnapshot}
                <p class="empty-copy">Native daemon scan unavailable in browser preview.</p>
              {:else if daemonProcesses.length === 0}
                <p class="empty-copy">No swarm-ui daemon or dev-support processes are visible.</p>
              {:else}
                <div class="stack-list compact-list">
                  {#each daemonProcesses as row (row.daemon_key)}
                    <div class="daemon-row" class:warn={row.stale}>
                      <div>
                        <strong>{row.label}</strong>
                        <span>{row.status} · pid {row.pid} · {formatDaemonRuntime(row.elapsed_seconds)}</span>
                      </div>
                      <code>{row.listening_ports.length ? `:${row.listening_ports.join(', :')}` : row.parent_label ?? 'no parent'}</code>
                    </div>
                  {/each}
                </div>
              {/if}
...
<style>
  ...
  .daemon-row.warn {
    border-color: color-mix(in srgb, var(--status-stale, #f9e2af) 64%, transparent);
    background: color-mix(in srgb, var(--status-stale, #f9e2af) 9%, transparent);
  }
