...
<script lang="ts">
  ...
  async function launchAgentProfile(profile: AgentProfile): Promise<boolean> {
    ...
    return launchResolved({
      ...
      scope: profile.scope || launchScope,
      label: profile.label,
      name: profile.nodeName,
      bootstrapInstructions: buildAgentProfilePrompt(profile) || undefined,
      scopeSource: profile.scope ? 'Agent Profile scope override' : launchScopeSourceCopy,
      commandSource: describeLaunchCommandSource(config.commandSource, null, true),
      launchDefaults: {
        harness: config.harness,
        role: config.role,
        scope: profile.scope || explicitScopeOverride,
      },
    });
  }
  ...
  <div class="body">
    <section class="block surface surface-primary quick-launch">
      ...
      <div class="form-group">
        <label for="quick-working-dir">Working dir</label>
        <input
          id="quick-working-dir"
          type="text"
          class="input mono"
      ...
      <div class="scope-truth-card" class:warning={Boolean(launchScopeWarningCopy)}>
        <div>
          <span>Launch truth</span>
          <strong>{launchScopeSourceCopy}</strong>
        </div>
        <p>
          Directory <code>{workingDir || 'not set'}</code>
          · Feed <code>{$activeScope ?? 'no active feed'}</code>
          · Registers <code>{launchScope || 'unresolved'}</code>
        </p>
        {#if launchScopeWarningCopy}
          <small>{launchScopeWarningCopy}</small>
        {/if}
      </div>
        ...
        <div class="quick-agent-grid" aria-label="Saved agents">
          {#each quickProfileSummaries as profile (profile.id)}
            ...
            >
              <button
                type="button"
                class="quick-agent-emoji"
                title="Customize role emoji"
                on:click={() => {
                  const match = $agentProfiles.find((entry) => entry.id === profile.id);
                  if (match) openRolePicker(match);
                }}
              >
              ...
              >
                ...
                <small title={profile.workingDirectory}>
                  {profile.workingDirectory || workingDir || 'Uses current working dir'}
                </small>
              </button>
            </div>
          {/each}
      ...
      <p class="hint command-source">
        Launch channel: <strong>{launchChannelCopy}</strong>
      </p>
      {#if launchScopeWarningCopy}
...
<style>
  ...
  .quick-agent-emoji {
    ...
    display: grid;
    place-items: center;
    border: 1px solid rgba(0, 240, 96, 0.72);
    border-radius: 6px;
    ...
    flex-shrink: 0;
    box-shadow:
      0 0 0 1px rgba(0, 240, 96, 0.16),
      0 0 14px rgba(0, 240, 96, 0.24);
  }
  ...
  .quick-agent-emoji:hover {
    border-color: rgba(0, 240, 96, 0.94);
    background: rgba(0, 0, 0, 0.78);
  }

  .quick-agent-card.fullAccess.selected .quick-agent-emoji {
  ...
  .quick-agent-card.fullAccess.selected .quick-agent-emoji > span {
    color: #ffffff;
  }
  ...
  :global([data-theme="tron-encom-os"]) .quick-agent-card.fullAccess.selected .quick-agent-emoji {
