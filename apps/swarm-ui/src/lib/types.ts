...
export type NodeType = 'instance' | 'pty' | 'bound' | 'browser' | 'app' | 'mission';

export type EdgeType = 'connection';

export type AgentCardMode = 'summary' | 'work' | 'context' | 'terminal';
...
export interface CanvasAppSurface {
  id: string;
  appId: 'chrome' | 'notes' | 'obsidian' | string;
  name: string;
  detail: string;
  icon: string;
  scope: string | null;
  status: 'open' | 'launched' | 'closed' | 'unknown';
  createdAt: number;
  updatedAt: number;
  document: CanvasAppDocument | null;
}

export interface CanvasMission {
  id: string;
  projectId: string;
  scope: string | null;
  root: string;
...
export interface CanvasAppDocument {
  assetId: string;
  projectId: string;
  kind: 'note' | 'plan';
  title: string;
...
export interface ShellSpawnResult {
  pty_id: string;
  /**
   * Present only for swarm-aware harness launches.
 ...
 * so the frontend can auto-type it into the new PTY,
...
export interface DaemonProcessRow {
  daemon_key: string;
  daemon_kind: string;
  label: string;
  pid: number;
...
export interface SystemLoadSnapshot {
  ...
  daemon_processes: DaemonProcessRow[];
...
export type XYFlowNode = FlowNode<SwarmNodeData, 'terminal' | 'browser' | 'appSurface' | 'mission'>;
...
export type SwarmNodeData = Record<string, unknown> & {
  ...
  /** Native or external app surface rendered on the canvas. */
  appSurface: CanvasAppSurface | null;
  /** Canvas-first mission/todo board rendered as a draggable node. */
