...
export function buildAnalyzeSummaryCards(snapshot: SystemLoadSnapshot): AnalyzeSummaryCard[] {
  return [
    ...
    {
      label: 'Daemon Radar',
      value: `${snapshot.daemon_processes.length}`,
      detail: `${snapshot.daemon_processes.filter((row) => row.stale).length} stale detached`,
    },
    {
...
export function buildAnalyzeOverlayModel(
  ...
  const daemonCount = snapshot.daemon_processes.length;
  const externalCount = snapshot.external_burden.length;
  const burdenTotal = Math.max(1, sessionCount + helperCount + daemonCount + externalCount);
  ...
  return {
    ...
    burdenSegments: [
      ...
      },
      {
        label: 'Swarm Daemons',
        value: daemonCount,
        percent: (daemonCount / burdenTotal) * 100,
      },
      {
