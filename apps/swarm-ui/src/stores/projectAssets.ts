...
export async function createProjectNoteAsset(
  projectId: string,
  title: string,
  content?: string,
): Promise<ProjectAsset> {
  const id = projectId.trim();
  const noteTitle = title.trim() || 'Canvas Note';
  if (!id) throw new Error('project id is required');

  const saved = await invoke<ProjectAsset>('ui_create_project_note_asset', {
    projectId: id,
    title: noteTitle,
    content: content?.trim() || null,
  });
  const normalized = normalizeProjectAsset(saved);
  if (!normalized) {
    throw new Error('created note asset response was invalid');
  }
  projectAssets.upsert(normalized);
  return normalized;
}

export async function updateProjectNoteAssetContent(
  assetId: string,
  content: string,
): Promise<ProjectAsset> {
  const id = assetId.trim();
  ...
  const saved = await invoke<ProjectAsset>('ui_update_project_note_asset_content', {
    assetId: id,
    content,
  });
  const normalized = normalizeProjectAsset(saved);
  if (!normalized) {
    throw new Error('updated note asset response was invalid');
  }
  projectAssets.upsert(normalized);
  return normalized;
}

export async function openProjectAssetPath(assetId: string): Promise<void> {
  const id = assetId.trim();
  if (!id) throw new Error('asset id is required');
  await invoke('ui_open_project_asset_path', { assetId: id });
}
