...
export const DEFAULT_LAUNCH_PROFILES: LaunchProfile[] = [
  ...
  },
  {
    id: 'hermes-nous',
    name: 'Hermes (Nous)',
    description: 'Nous Hermes Agent terminal session with swarm bootstrap.',
    harness: 'hermes',
    command: 'hermes --tui',
    trustPosture: 'trusted-local',
    defaultRole: 'researcher',
  ...
  },
  {
    id: 'openclaw-local',
    name: 'OpenClaw Local',
    description: 'OpenClaw local terminal UI through openclaw chat.',
    harness: 'openclaw',
  ...
  },
  {
    id: 'opencode-local',
    name: 'OpenCode',
    description: 'OpenCode terminal coding agent with swarm bootstrap.',
    harness: 'opencode',
    command: 'opencode',
    trustPosture: 'trusted-local',
    defaultRole: 'implementer',
...
export function normalizeLaunchProfiles(value?: Partial<LaunchProfile>[] | null): LaunchProfile[] {
  ...
  if (normalized.length === 0) return [...DEFAULT_LAUNCH_PROFILES];

  const defaultsById = new Map(DEFAULT_LAUNCH_PROFILES.map((profile) => [profile.id, profile]));
  const customProfiles: LaunchProfile[] = [];
  for (const profile of normalized) {
    if (defaultsById.has(profile.id)) {
      defaultsById.set(profile.id, profile);
    } else {
      customProfiles.push(profile);
    }
  }
  return [...defaultsById.values(), ...customProfiles];
}
