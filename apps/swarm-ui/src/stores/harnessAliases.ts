...
export type HarnessName = 'claude' | 'codex' | 'hermes' | 'openclaw' | 'opencode';

export type HarnessAliases = Record<HarnessName, string>;

const STORAGE_KEY = 'swarm-ui.harness-aliases';

export const HARNESS_NAMES: HarnessName[] = ['claude', 'codex', 'hermes', 'openclaw', 'opencode'];
...
function sanitizeAlias(harness: HarnessName, value: unknown): string {
  ...
  if (harness === 'hermes' && trimmed === 'hermes') return DEFAULT_ALIASES.hermes;
...
function normalize(value?: Partial<HarnessAliases> | null): HarnessAliases {
  return {
    ...
    hermes: sanitizeAlias('hermes', value?.hermes),
    openclaw: sanitizeAlias('openclaw', value?.openclaw),
