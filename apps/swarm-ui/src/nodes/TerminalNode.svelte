...
<script lang="ts">
  ...
  import { buildAgentCopyAllText, terminalBufferToText } from '../lib/agentCopyAll';
  import {
    compactNodeIds,
    requestNodeWorkspace,
    toggleCompactNode,
  } from '../lib/app/nodeWindowState';
  import { getPtyBuffer } from '../stores/pty';
  ...
  let copyAllState: 'idle' | 'copying' | 'copied' | 'failed' = 'idle';
  let copyAllResetTimer: number | null = null;

  onDestroy(() => {
    if (copyAllResetTimer !== null) {
  ...
  function markCopyAllState(state: typeof copyAllState): void {
    copyAllState = state;
    if (copyAllResetTimer !== null) {
      window.clearTimeout(copyAllResetTimer);
  ...
  async function handleCopyAll() {
    if (copyAllState === 'copying') return;
    markCopyAllState('copying');
    try {
      const terminalBuffer = ptyId ? await getPtyBuffer(ptyId) : new Uint8Array();
      const terminalText = terminalBufferToText(terminalBuffer);
      await navigator.clipboard.writeText(buildAgentCopyAllText(data, terminalText));
      markCopyAllState('copied');
    } catch (err) {
      console.error('[TerminalNode] failed to copy agent diagnostics:', err);
      markCopyAllState('failed');
    }
  }
