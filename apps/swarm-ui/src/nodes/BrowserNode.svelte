<script lang="ts">
  ...
  import type { BrowserCatalog, SwarmNodeData } from '../lib/types';
  ...
  function emitCatalog(catalog: BrowserCatalog, message: string): void {
    window.dispatchEvent(new CustomEvent('swarm-browser-catalog', {
      detail: {
        catalog,
        focusContextId: context?.id ?? null,
        message,
      },
    }));
  }

  async function refreshContext(event: MouseEvent): Promise<void> {
    event.preventDefault();
    event.stopPropagation();
    if (!context || nodeBusy) return;
    nodeBusy = true;
    nodeError = '';
    try {
      const catalog = await invoke<BrowserCatalog>('ui_refresh_browser_context', {
        scope: context.scope,
        contextId: context.id,
      });
      emitCatalog(catalog, 'Browser surface checked for stale state.');
    } catch (err) {
  ...
  async function deleteContext(event: MouseEvent): Promise<void> {
    event.preventDefault();
    event.stopPropagation();
    if (!context || nodeBusy) return;
    const ok = await confirm({
      title: 'Delete browser surface',
      message: `Close and remove this managed Chrome surface from the canvas?\n${url || context.startUrl}`,
    ...
    try {
      ...
      emitCatalog(catalog, 'Browser surface deleted from the canvas.');
    } catch (err) {
      nodeError = err instanceof Error ? err.message : String(err);
    } finally {
      nodeBusy = false;
...
>
  ...
  <header class="browser-node-header">
    <span class="browser-node-icon" aria-hidden="true">
      <img src={chromeLogo} alt="" />
    </span>
    <div>
      <span>Chrome App Surface</span>
    ...
    <div class="browser-node-badges">
      ...
      <button type="button" disabled={nodeBusy || !context} title="Check for stale browser state" on:click={refreshContext}>
        Check
      </button>
      <button type="button" class="danger" disabled={nodeBusy || !context} title="Close and delete this browser surface" on:click={deleteContext}>
        Delete
      </button>
    </div>
  </header>
  ...
  <section class="browser-node-body">
    {#if nodeError}
      <div class="browser-node-error">{nodeError}</div>
    {/if}
...
<style>
  ...
  .browser-node-icon {
    ...
    box-shadow: 0 0 18px rgba(66, 133, 244, 0.24);
  }
  ...
  .browser-node-badges button:hover {
    border-color: rgba(255, 255, 255, 0.54);
    color: rgba(255, 255, 255, 0.96);
  }

  .browser-node-badges button.danger {
    border-color: rgba(255, 120, 120, 0.36);
    color: rgba(255, 176, 176, 0.88);
  }

  .browser-node-badges button:disabled {
    cursor: default;
    opacity: 0.5;
  }
