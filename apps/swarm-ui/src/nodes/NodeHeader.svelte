...
<script lang="ts">
  ...
  import { providerLogoForProvider } from '../lib/agentProviders';
  ...
  export let copyAllState: 'idle' | 'copying' | 'copied' | 'failed' = 'idle';
  /**
  ...
  $: providerLogo = providerLogoForProvider(providerLabel);
  ...
  $: closeLabel = !canClose
    ...
    : isClearableStalePty
      ? 'Clear stale node'
      : 'Close agent window';
  $: copyAllTitle = copyAllState === 'copied'
    ? 'Copied agent config and terminal text'
    : copyAllState === 'failed'
      ? 'Copy failed'
      : 'Copy agent config and terminal text';
...
>
  ...
  >
    ...
    {/if}
    <span class="node-agent-emoji" aria-hidden="true">
      {#if providerLogo}
  ...
  <div class="node-controls">
    <button
      class="copy-all"
      class:copied={copyAllState === 'copied'}
      class:failed={copyAllState === 'failed'}
      title={copyAllTitle}
      aria-label={copyAllTitle}
      disabled={copyAllState === 'copying'}
      on:click|stopPropagation={() => dispatch('copyAll')}
    >
      Copy All
    </button>
