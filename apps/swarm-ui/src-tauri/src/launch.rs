...
/// (claude/codex/hermes/openclaw/opencode) the backend pre-creates a swarm instance row,
...
mod tests {
    ...
    fn is_harness_shell_role_recognizes_harnesses() {
        ...
        assert!(is_harness_shell_role("hermes"));
        assert!(is_harness_shell_role("openclaw"));
    ...
    fn launch_config_includes_default_roles() {
        ...
        assert!(config.is_known("builder"));
        assert!(config.is_known("operator"));
        assert!(config.is_known("designer"));
        assert!(config.is_known("majordomo"));
        assert!(config.is_known("qa"));
        assert!(config.is_known("tester"));
    ...
    fn validate_role_accepts_common_aliases() {
        ...
        assert_eq!(
            validate_role(Some("generalist"), &config).unwrap(),
