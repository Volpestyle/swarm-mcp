...
fn path_preview(raw_path: &str) -> String {
    let mut parts: Vec<&str> = raw_path.split(':').filter(|part| !part.is_empty()).take(6).collect();
    if parts.is_empty() {
        return String::new();
    }
...
fn preflight_slash_command(
    ...
    let blocker = if executable_file(&resolved) {
        ...
        Some(format!("Launch command is not executable: {}", resolved.display()))
    } else {
        Some(format!("Launch command does not exist: {}", resolved.display()))
    };
...
fn cdp_get_body(host: &str, port: i64, path: &str) -> Result<String, AppError> {
    ...
    stream.read_to_string(&mut response).map_err(|err| {
        AppError::Operation(format!("failed to read browser CDP response: {err}"))
    })?;
...
fn open_browser_context_for_scope(
    ...
    let mut child = Command::new(DEFAULT_CHROME_PATH)
...
pub fn ui_open_browser_context(
    ...
    open_browser_context_for_scope(&scope, start_url)
}
...
pub fn ui_import_front_chrome_tab(scope: String) -> Result<BrowserCatalog, AppError> {
    ...
    open_browser_context_for_scope(&scope, tab.url)
}
...
mod tests {
    ...
    fn launch_preflight_blocks_missing_command() {
        let result = ui_preflight_launch_command(
            "definitely-not-swarm-ui-command-xyz".into(),
            None,
        )
        .unwrap();
        assert!(!result.ok);
