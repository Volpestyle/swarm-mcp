...
Slice 0 through Slice 8C are complete for the local dev surface. Slice 7D is complete for the native restart survivor/rebind/kill backbone. Slice 8D native background-control proof is accepted, with human-visible installed click/reopen and notifications tracked as follow-up. The May 2 debug-bundle packaging blocker was cleared by targeted Cargo artifact cleanup; the remaining native gap is human-visible click/screenshot proof, not packaging.
...
- Build refresh on 2026-05-02: root `bun run check` passed, and targeted cleanup of stale Tauri/Cargo artifacts cleared the `crate tauri required to be available in rlib format` failure. `bunx tauri build --debug` now bundles `/Users/mathewfrazier/Desktop/swarm-mcp-lab/target/debug/bundle/macos/swarm-ui.app`.
