...
The owner advertises a compact API version, schema version, skill contract,
...
New owner configs include `version: 1`. Unversioned configs from earlier builds
...
a stale executable/config path. Use the same build for owner and adapters,
...
See [installation](installation.md) for the doctor and
[migration and cutover](migration-cutover.md) for moving a live profile.
