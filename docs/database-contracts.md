...
`apps/swarm-server/migrations/0001_initial.sql` is not the `swarm.db` schema. It owns the separate auth database used for devices, tokens, pairing codes, rate limits, and audit events.
...
2. The spawned process receives `SWARM_MCP_INSTANCE_ID` and related scope/directory env vars.
...
If a pending row is pruned before adoption, the MCP server may insert a fresh adopted row using the same preassigned id so the PTY binding remains stable.
