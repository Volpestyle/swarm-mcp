...
The current package serves only the nine-tool coordinator API, through both
`swarm-mcp` and `swarm-coordinator-mcp`. Trusted runtime enrollment is required.
It uses a separate `coordination.db`; no entrypoint silently opens or converts
`swarm.db`. Use [offline migration](migration-cutover.md) for existing data.

Historical caller concepts map to the current interface below. These are migration
routes, not aliases. Runtime means trusted launcher/host integration.
...
| `remove_instance` | Runtime lifecycle management; no replacement yet (VUH-1339) |
...
| `broadcast` | Core `message.announce` explicit audience; no broadcast tool yet |
...
| `get_file_lock` | Core reservation query exists; runtime exposure remains incomplete |
...
Tools return `{data}` on success and `{error}` on failure as
structuredContent and JSON text. MCP `isError` determines success. Parsers must use this envelope; do not parse English success text or assume snake_case
...
Held and immediate event reads return at most 20 relevant events, skipping
