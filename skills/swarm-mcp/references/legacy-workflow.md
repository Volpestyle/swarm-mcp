...
If `bootstrap` returns `work_tracker`, use only that configured tracker for this repo/scope and identity. Do not pick a tracker just because an MCP happens to be loaded; load `legacy-work-trackers.md` when tracker linkage matters.
...
- `planner`: load `planner.md` and register with `identity:<work|personal> role:planner`
- `implementer`: load `implementer.md` and register with `identity:<work|personal> role:implementer`
- `reviewer`: load `reviewer.md` and register with `identity:<work|personal> role:reviewer`
- `researcher`: load `researcher.md` and register with `identity:<work|personal> role:researcher`
...
|-------|-----------|-----------|
| Bootstrap and registration fields | `bootstrap.md` | You need to decide `directory`, `scope`, `file_root`, or `label` |
| Planner workflow | `planner.md` | The session should plan, delegate, monitor, or recover work |
| Implementer workflow | `implementer.md` | The session should claim tasks and edit code |
| Reviewer workflow | `reviewer.md` | The session should review completed work or inspect risk |
| Researcher workflow | `researcher.md` | The session should investigate and publish findings |
| Gateway workflow | `gateway.md` | The session has `mode:gateway` and needs to dispatch, spawn, place, or monitor workers |
| Work tracker linkage | `legacy-work-trackers.md` | Work should be linked to Linear, Jira, GitHub Issues, or another configured tracker |
| KV and shared coordination state | `coordination.md` | You need to read/write `progress/`, `plan/`, `owner/`, queue, or handoff keys |
| Specialists, generalists, and team conventions | `roles-and-teams.md` | You need to route work by `role:` or `team:` labels |
...
| `swarm-mcp` CLI reference | `cli.md` | You are about to write or invoke a helper script, inspect swarm state from a plain terminal, or control `swarm-ui` through the CLI |
...
- In gateway/lead mode, no live worker is a spawn problem, not a native-subagent fallback. Load `gateway.md` before dispatching, spawning, setting `placement`, or using `completion_wait_seconds`. Worker/generalist sessions do not spawn new workers; they request tracked work, message the planner/gateway, or continue locally when safe.
